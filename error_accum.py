...
class WeatherErrorAccumulationMetricJAX:
    ...
    def compute_delta_for_timestep(self, time) -> float:
        """
        Compute the error accumulation metric (delta) for a specific time step.

        Parameters:
        - time: The time step at which to compute the delta.
        ...
        gen_samples = self.gen_data[time]  # Shape: (n_samples, ...)
        cts_samples = self.cts_data[time]  # Shape: (...)
        ...
        try:
            ...
            cts_samples_flat = cts_samples.flatten()

            # Compute per-location mean and std over the generator samples
            gen_mean = jnp.mean(gen_samples_flat, axis=0)
            gen_std = jnp.std(gen_samples_flat, axis=0) + 1e-6  # Avoid zero std

            # Compute mean and std over the CTS samples
            cts_mean = jnp.mean(cts_samples_flat)
            cts_std = jnp.std(cts_samples_flat) + 1e-6

            # Compute KL divergence for each location
            kl_values = self._kl_divergence(gen_mean, gen_std, cts_mean, cts_std)

            # Average KL divergences over all spatial locations
            delta = float(jnp.mean(kl_values))
